...
function checkuserlogin(){
	if($("#my_user_top").length>0 || $("#my_shop_today_sign").length>0){
		if(g_irefer){
...
var login_w='440px';
$(function(){
	if($(".login_wrap .img_adspace").size()>=1){
		login_w='640px';
	}else{
		login_w='440px';
		$(".login_wrap").css("margin-left", "0px")
	}
})
function openlogin(){
...
function reg_success(synurl){
	layer.close();
	if(synurl != ""){
		window.location.href = synurl+"&refer="+encodeURIComponent(window.location.href);
	}else{
...
function checkuserlogin_back(data){
	if(data.code == 1){
		if($("#my_user_top").length > 0){
			var userinfo = '<div class="userlab">';
	        userinfo += '<div class="li username">';
	        userinfo += '<div class="user">';
	        userinfo += '<div class="img">';
	        userinfo += '<img src="'+data.user.avatarstr+'" />';
	        userinfo += '</div>';
	        userinfo += '<div class="Popup userpopup" >';
	        userinfo += '<div class="cur"></div>';
	        userinfo += '<div class="usname">';
	        userinfo += '<div class="tximg">';
	        userinfo += '<img src="'+data.user.avatarstr+'" />';
	        userinfo += '</div>';
	        userinfo += '<div class="name_tex">';
	        userinfo += '<div class="name_"><a href="'+myuserhost+(typeof(data.user.is_mobile) != "undefined" && data.user.is_mobile == 1 ? '/setting' : '/setting/binding')+'" target="_blank" class="nam">'+data.user.nickname+'</a>';
	        userinfo += '<a href="javascript:void(0);" class="exit" onclick="logout_submit()">退出</a></div>';
	        userinfo += '<div class="p">我的积分：'+data.user.integral+'</div>';
	        userinfo += '</div>';
	        userinfo += '<a href="'+myuserhost+'/setting" class="btn_">设置</a>';
	        userinfo += '<div class="lab_a">';
	        userinfo += '<a href="'+myuserhost+'/setting/binding" target="_blank" class="phone'+(typeof(data.user.is_mobile) != "undefined" && data.user.is_mobile == 1 ? ' on' : '')+'"><i class="hduser"></i></a>';
	        userinfo += '<a href="'+myuserhost+'/setting/binding" target="_blank" class="weixin'+(typeof(data.user.is_wechat) != "undefined" && data.user.is_wechat == 1 ? ' on' : '')+'"><i class="hduser"></i></a>';
	        userinfo += '<a href="'+myuserhost+'/setting/binding" target="_blank" class="qq'+(typeof(data.user.is_qq) != "undefined" && data.user.is_qq == 1 ? ' on' : '')+'"><i class="hduser"></i></a>';
	        userinfo += '<a href="'+myuserhost+'/setting/binding" target="_blank" class="weibo'+(typeof(data.user.is_sina) != "undefined" && data.user.is_sina == 1 ? ' on' : '')+'"><i class="hduser"></i></a>';
	        userinfo += '</div>';
	        userinfo += '</div>';
	        userinfo += '<ul class="bq_list">';
	        userinfo += '<li><a href="'+myuserhost+'/orders/"><div class="num">'+data.user.order_total+'</div><p>订单</p></a></li>';
	        userinfo += '<li><a href="'+myuserhost+'/comment"><div class="num">'+data.user.ct_total+'</div><p>评论</p></a></li>';
	        userinfo += '<li><a href="'+myuserhost+'/collection"><div class="num">'+data.user.favorite_total+'</div><p>收藏</p></a></li>';
	        userinfo += '</ul>';
	        userinfo += '<div class="signbox">';
	        userinfo += '<div class="signlab">';
	        userinfo += '<div class="p">累计签到'+data.mysign.sign_total+'天</div>';
	        userinfo += '<div class="p">';
	        userinfo += '<span>目前连续签到<u>'+data.mysign.contin_sign_total+'</u>天</span>';
	        userinfo += '<span>经验<u><i>+</i>'+(data.mysign.contin_sign_total > 7 ? 20 : 10)+'</u></span>';
	        userinfo += '</div>';
	        userinfo += '</div>';
	        userinfo += '<div class="signline">';
	        userinfo += '<span>周进度</span>';
	        userinfo += '<div class="star"><div class="line" style="width: '+parseInt(data.mysign.week_sign_total*100/7)+'%;"></div></div>';
	        if(data.mysign.has_sign == 1){
	        	userinfo += '<span class="tab">已签到</span>';
	        }else{
		    	userinfo += '<div class="tab sign_a" onclick="user_signday()">签到</div>';
		    }
	        userinfo += '</div>';
	        userinfo += '</div>';
	        userinfo += '</div>';
	        userinfo += '</div>';
	        userinfo += '<div class="name">'+data.user.nickname+'</div>';
	        userinfo += '</div>';
	        var msglistlen = typeof(data.user.msglist) == "undefined" ? 0 : data.user.msglist.length;
	        if( msglistlen > 0){
		        userinfo += '<div class="li tx">';
		        userinfo += '<div class="txlab">';
		        userinfo += '<i class="hduser icotx"></i>';
		        userinfo += '<div class="Popup txpopup" >';
		        userinfo += '<div class="cur"></div>';
		        userinfo += '<div class="titbt">';
		        userinfo += '<div class="bt"><i class="hduser ico"></i>我的消息</div>'
		        userinfo += '<a href="'+myuserhost+'/msg/" target="_blank" class="more">查看更多</a>';
		        userinfo += '</div>';
		        userinfo += '<div class="list scroll">';
		        for(var imsg=0; imsg < msglistlen; imsg++){
					var msgtype = data.user.msglist[imsg]['type'];
					var msglist_type = "消息";
					var msglist_a = myuserhost+'/msg/';
					if(msgtype == 2 || msgtype == 3 || msgtype == 4 || msgtype == 7){
						msglist_a = myuserhost+'/comment/';
						msglist_type = "评论";
					}else if(msgtype == 1 || msgtype == 5 || msgtype == 6 || msgtype == 8 || msgtype == 9 || msgtype == 11 || msgtype == 12 || msgtype == 13 || msgtype == 14 || msgtype == 15 || msgtype == 16 || msgtype == 22){
						msglist_a = myuserhost+'/integral/';
					}
					...
					userinfo += '<a href="'+msglist_a+'" target="_blank" '+(imsg == 0 ? ' class="on" ': '')+' onclick="clearUserMsg('+data.user.msglist[imsg]['id']+')"><i>['+msglist_type+']</i><p><span>'+msglist_msg+'</span></p></a>';
				}
		        userinfo += '</div>';
		        userinfo += '<a href="javascript:void(0);" class="closelis" onclick="clearUserallMsg()">清空消息</a>';
		        userinfo += '</div>';
		        userinfo += '</div>';
		        userinfo += '<span>'+data.user.msg_total+'</span>';
		        userinfo += '</div>';
	    	}
	        userinfo += '<div class="li sign">';
	        if(data.mysign.has_sign == 1){
	        	userinfo += '<div class="lab">已签到</div>';
	    	}else{
	    		userinfo += '<div class="lab sign_a" onclick="user_signday()">签到</div>';
	    	}
	        userinfo += '</div>';
		    userinfo += '<div class="li task">';
		    userinfo += '<div class="lab">每日任务</div>';
		    userinfo += '<div class="Popup taskpopup" >';
		    userinfo += '<div class="cur"></div>';
		    userinfo += '<div class="titbt">';
		    userinfo += '<div class="bt"><i class="hduser ico"></i>每日任务</div>';
		    userinfo += '</div>';
			userinfo += '<div class="Popupapp">';
            userinfo += '<div class="img"><img src="'+myuserhost+'/appdownqr"></div>';
            userinfo += '<p>请使用手机扫描二维码 <br/> 前往3DM官方App完成每日任务</p>';
            userinfo += '</div>';        
	        userinfo += '</div>';
	       	userinfo += '</div>';
	        userinfo += '</div>';
		...
		if($("#my_shop_today_sign").length > 0){
	        if(data.mysign.has_sign == 1){
	        	$("#my_shop_today_sign").html('<div class="lab">已签到</div>');
	    	}else{
	    		$("#my_shop_today_sign").html('<div class="lab sign_a" onclick="user_signday()">签到</div>');
	    	}
		}
		//需要刷新评论的地方
		try {
	    ...
	    //唤起需要更新的
	    try {
		    if(typeof callshopuserinfo2 === "function") {
		        callshopuserinfo2();
		    }
	    }catch(e){ }
	}
...
function reg_submit_back(data, areacode, mobile, pwd, cpwd, vcode){
	if(data.code == 1){
		...
		if(data.isreg == 1){
			layer.msg("注册成功");
			synurl = data.data != "undefined" && data.data.synurl !="undefined" ? data.data.synurl : "";
		}else{
			layer.msg("请先退出再注册");
        }
        setTimeout("reg_success('"+synurl+"')",500);
    }else{
...
function user_signday(){
	if(g_irefer){
		var url = myuserhost + "/jsonpapi/subsign?irefer="+encodeURIComponent(top.location.href);
		$.ajax({
        	url: url,
        	async:false,
        	type: "GET",
        	...
        	success: function(data){
            	alert(data.msg);
            	if(data.code == 1){
            		$(".sign_a").removeAttr("onlick");
            		$(".sign_a").html("已签到");
            	}
        	}
    	});
	}else{
		var url = myuserhost + "/api/subsign?irefer="+encodeURIComponent(top.location.href);
		$.ajax({
        	url: url,
        	async:false,
        	type: "POST",
...
function user_gettask(obj, type){
	if(g_irefer){
		var url = myuserhost + "/jsonpapi/gettaskgrowth?irefer="+encodeURIComponent(top.location.href);
		$.ajax({
        	url: url,
        	async:false,
        	type: "GET",
		...
		var url = myuserhost + "/api/gettaskgrowth?irefer="+encodeURIComponent(top.location.href);
		$.ajax({
        	url: url,
        	async:false,
        	type: "POST",
