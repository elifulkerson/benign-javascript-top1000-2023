...
try {
    if (typeof Suggester == "undefined") {    
            ...
            try {
                ...
                e.minStringLengthToDisplaySuggestion || (e.minStringLengthToDisplaySuggestion = 1),
                    ...
                    this.setDefaultContextData = function(defaultContext){         //Function to call defcon
                        0 != defaultContext.cat && (this.contextData = defaultContextData(asgv.domain + "suggest/suggester.php", defaultContext.cat, defaultContext.mcat))
                   },
                    this.setContext=function(pageContext){        //function to set the Context for the Page                        
                     
                           
                        if (Array.isArray(pageContext.cat)) {
                           
                            this.context.cat = pageContext.cat;
                        } 
                        else 
                        {
                            this.context.cat = parseInt(pageContext.cat);
                            if (isNaN(this.context.cat))
                            {
                                this.context.cat = 0;
                            }
                        ...
                        if (Array.isArray(pageContext.mcat)) {
                            
                            this.context.mcat = pageContext.mcat;
                        } 
                        else 
                        {
                            this.context.mcat = parseInt(pageContext.mcat);
                            if (isNaN(this.context.mcat))
                            {
                                this.context.mcat = 0;
                            }
        ...
        var cimjsv;
        var perks = ["prod_data", "mcat_data", "keyw_data", "city_data", "meta_data","rel_data","rel_prod"];
