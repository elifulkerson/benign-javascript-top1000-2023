...
function stateExternalIconType(link_element) {
    ...
    // Check if the link is internal or if it has already been set up. 
    // Save checking for external icon for below after the check for the light theme.
    // This is to correctly set the theme to light for those links that were incorrectly themed default in an earlier version of this code.
    if (isInternalSite(link_element.host) ||
        ...
        jQuery(link_element).hasClass('recent-content__link--twitter') ||
    ...
    if( jQuery(link_element).parent().hasClass('has-state-white-color') ||
        ...
        'rgb(255, 255, 255)' === jQuery(link_element).css('color')) {
        return 'light';
    } else if ( jQuery(link_element).find('span.icon-external').length !== 0 ||
...
function stateAddLinkExtras(linkSelector) {
    ...
    jQuery(linkSelector).each(function() {
        ...
        // caching checks for pre-existing icons
        let icon_pdf = jQuery(link_element).find('.icon-pdf');
        let icon_external = jQuery(link_element).find('.icon-external');
        let icon_external_white = jQuery(link_element).find('.icon-external__white');
        let br_tag = jQuery(link_element).find('br');

        if ( 'pdf' === file_type && 0 === icon_pdf.length) {
        ...
        } else if('light' === external_icon_type &&
                    0 !== icon_external.length) {
                        // remove pre-existing icon-external span class and replace it with the icon-external__white
                        icon_external.remove();
                        let external_link_icon = "<span class='icon-external__white'></span>";
                        if (0 < br_tag.length) {
                            br_tag.before(external_link_icon);
                            return;
                        } 
