...
function kokosMaker(page)
{
	...
	if(length_troom > 0)
	{
		...
		{
			/*for(var i = 0; i < length_troom; i++) {
				if(troom[i] == str[key] && fulldate >= meatball_start) 
				{
					//urlApi = kokosApiRd;
					urlApi = kokosApiCt;
				}
			...
			//console.log(document.getElementById(position));
				if (document.getElementById(position).innerHTML.trim() === '')
                {
					if(obj['items'][0]['source']['type_file'] != "Script"){
						...
						{
							if(check_mobile >= 0){
								document.getElementById(position).innerHTML = '<div class="ads_box_type1 m-b-20 '+cls_toggle+'"><iframe src="'+base_url+jsapp+'/show_ifrm_app/'+obj['items'][0]['bingsu_id']+is_mobile+'" width="100%" height="265" frameborder="0" title="ads"></iframe></div>'; 
							}
							else
							{
								document.getElementById(position).innerHTML = '<div class="ads_box_type1 ads_with_iframe m-b-20 '+cls_toggle+'"><iframe src="'+base_url+jsapp+'/show_ifrm_app/'+obj['items'][0]['bingsu_id']+is_mobile+'" width="100%" height="265" frameborder="0" class="ads_box_iframe" title="ads"></iframe></div>'; 
							}
						}
...
window.addEventListener("load", function load(event)
{
	...
	function AdsScaler()
	{
		...
		{
			if(adsBox_1[i].offsetWidth >= 1120)
			{
				scaleX = (adsBox_1[i].offsetWidth - 150) / 970;
				adsBox_1[i].style.height = ((adsBox_1[i].offsetWidth - 100 )/ 970) * 282 + "px";	
			}
			if(adsBox_1[i].offsetWidth >= 1080 && adsBox_1[i].offsetWidth < 1119)
			{
				scaleX = (adsBox_1[i].offsetWidth - 100) / 970;
				adsBox_1[i].style.height = ((adsBox_1[i].offsetWidth - 100 )/ 970) * 282 + "px";	
			}
			if(adsBox_1[i].offsetWidth >= 970 && adsBox_1[i].offsetWidth < 1079)
			{
				scaleX = (adsBox_1[i].offsetWidth - 100) / 970;
				adsBox_1[i].style.height = ((adsBox_1[i].offsetWidth - 100 )/ 970) * 282 + "px";	
			}
			if(adsBox_1[i].offsetWidth < 970)
			{
				...
				adsBox_1[i].style.height = (adsBox_1[i].offsetWidth / 970) * 282 + "px";	
			}
			if(adsBox_1[i].style.height >= 282 + "px")
			{
				adsBox_1[i].style.height = 282 + "px"
			}
			if(scaleX <= 0.34)
			{
				scaleX = 0.34;
