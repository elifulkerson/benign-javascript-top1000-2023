...
try {
	...
	var appvars = {
		...
		push_content: url.searchParams.get("push_content") || Cookies.get('push_content'),
		utm_content: url.searchParams.get("utm_content") || Cookies.get('utm_content'),
	...
	var ad = '', an = '', o = '', ag = '', qo = '', rch = '', rtb = '', uniqueid = '', push_content = '';
	...
	if (appvars.push_content) push_content = '&push_content=' + encodeURIComponent(appvars.push_content);
...
jQuery(document).ready(function () {
	...
	// Sitespect AB - OGP-380 test to remove first image before GRS ads. ( START )
	if ( 1 === parseInt(Cookies.get('amg_r_featured')) ){
		let postContent = jQuery('.single-post .single-post-content-wrapper');
		if( postContent.children().eq(0).is("figure") ){
			postContent.children().eq(0).remove();
		}
	}
	// Sitespect AB - OGP-380 test to remove first image before GRS ads. ( END )

	// GDPR onetrust implementation through GTM, Open Toggle on privacy box click
	setTimeout(function(){
		if( jQuery("[href=#open-privacy-policy]").length ) {
			jQuery("[href=#open-privacy-policy]").click(function(e){
				if( typeof Optanon === 'undefined' || Optanon === null ) {
					return;
				}
	...
	if( amgGaObj.amgMagniteDisabled ){
		document.querySelectorAll('.global-site-magnite-ad,.home-page-ad').forEach(function(el) {
...
jQuery(window).load(function () {
	...
	if (typeof amgLeadGenerationAdObj !== "undefined") {
		...
		if (amgLeadGenerationAdObj.currentBlog.siteurl.indexOf('budgetsmart') !== -1) {
			sitePath = 'budgetsmart';
		}
	...
	$('.search-form').on('submit', function (e) {
		...
		if (search.length > 0 ) {
			if( 'none' !== $(this).find('.search-box').css('display') && $('.search-col').hasClass( 'search_open') ){
				$(this).find('.search-box').css('display', 'none')
				location.href = domain + '/web?q=' + search + ad + an + o + ag + push_content + qo + uniqueid;
			} else {
				$('.search-box').show();
			}
			// $('.search-box').val('');
			...
			$.getJSON(searchApi + '&lang=' + navigator.language + '&tag=wfp&limit=10&hi=1&q=' + query, function (d) {
				...
				d.suggestions.forEach(function (e) {
					...
					if( searchField.length ) {
						...
						searchFieldListItem.setAttribute('data-href', domain + '/web?q=' + search + rtb + o + push_content );
						searchFieldListItem.setAttribute( 'onclick', 'greensearchSubmit("'+search+'");' );
					} else {
						searchFieldListItem.setAttribute('href', domain + '/web?q=' + search + ad + an + o + ag + qo + uniqueid + push_content );
					}
...
$( document ).on( 'ready', function () {
    ...
    $( document ).on( 'click', '.link-share-button-svg', function ( event ) {
        ...
        if( $(this).hasClass('copy_questions_of_day') ){
            currentUrl+= '#questions_of_day'
        }
        let temp = $( '<input>' );
        $( 'body' ).append( temp );
...
/**
 * Implements CMARKETING-1581
 */
var bodyElement = document.querySelector('body');
if( bodyElement.classList.contains('single-post') ){
    var url = new URL(location.href);
    var utm_content = url.searchParams.get("utm_campaign") || "";
    googletag.pubads().setTargeting('utm_campaign', utm_content ).setTargeting( 'topic', amgGaObj.cat1 );
}
;
