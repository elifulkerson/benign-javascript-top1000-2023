...
var AsanaHelpers = {
  ...
  setGlobalClasses: function () {
    ...
    if (this.isLoggedIn()) {
      document.querySelector("html").classList.add("is-logged-in");
  ...
  signupModal: {
    init: function (options) {
      setTimeout(function () {
        ...
        if (signupBtn && closeTriggers) {
          const clickHandler = AsanaHelpers.signupModal.getSignupClickHandler(options);
    ...
    getSignupClickHandler: function (options) {
      return function (event) {
        ...
        try {
          ...
          } else if (options && options.triggeredFrom === 'pricing_basic_signup') {
            // For ungated_trials_skip_fomo_free_intent_autojoiner experiment:
            // Sends a signup request to determine experiment enrollment which decides
            // whether to display the Premium Reminder Modal or skip it
            AsanaHelpers.enterFreeSignupFlow();
            AsanaHelpers.signupWithEmail(form, options);
          } else {
  ...
  tier: {
     ...
     *
     * @return {"tier-0-free"|"tier-1-premium"|"tier1plus-premium"|"tier-2-enterprise"|null}
     */
     ...
     *
     * @return {"free"|"premium"|"business"|"enterprise"|null}
     */
    ...
    isTierPremium: function () {
      return this.getCurrentTier() === 'tier-1-premium';
    },
    ...
    isTierBusiness: function () {
      return this.getCurrentTier() === 'tier-1plus-premium';
    },
   ...
   * @param {string} [options.triggeredFrom] - A string identifying which component triggered the signup
  ...
  signupWithEmail: function (form, options) {
    ...
    var triggeredFrom = options && options.triggeredFrom;
    ...
    if (triggeredFrom) {
      submitData.static_site_source = triggeredFrom;
    }
    ...
    if (currentTierName) {
      // endpoint expects a name like "premium" instead of an id like "tier-1-premium"
      submitData.registered_with_tier = currentTierName;
    } else {
      //sets default (fallback) tier k/v pair in tier object to premium.
      AsanaHelpers.tier.defaultTier = 'tier-1-premium';
    ...
    */
    ...
    function handleSignupSuccess(responseData, submitData, isContentful) {
      // For ungated_trials_skip_fomo_free_intent_autojoiner experiment:
      // Users found to be in the control group or not enrolled will hit this block
      // and will be shown the Premium Reminder Modal
      if (responseData.show_premium_reminder) {
        var signupModalContainer = document.querySelector('#signup.modal .modal-container');
        signupModalContainer.classList.add('premium-reminder-modal-transition');
        return;
      }
      ...
      if (responseData.should_use_pin_verification) {
        AsanaStorage.setCookie("should_use_pin_verification", true, AsanaStorage.CATEGORY.NECESSARY);
      }
